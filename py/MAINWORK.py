...
#		self.methodmap = {"index": self.index, "test": self.test, "account": self.account, "profile": self.profile, "menu": self.menu, "cart": self.cart};
	...
	def index(self):
		...
		rowCritical= lambda tname: mappl(lambda x: sifu(x, "number", intf(x["actual"]) ) , [
		_sql.g1("select cast(sum(actual) as unsigned) as actual, cast(sum(target) as unsigned) as target from "+tname+" where date = {date} ", {"date": t2f("%Y-%m-%d", int_date)})]);
		...
		#rowSuperCritical= 
		#_sql.g1("select cast(sum(number) as unsigned) as number, from "+tbl_super_critical_coal_plant+" where date = {date} ", {"date": t2f("%Y-%m-%d", int_date)});
		self.jsdata["critical"] = mapp(lambda x: rowCritical(x), rCritical, None, lambda x: rCritical[x]);
		self.jsdata["super_critical"] = mapp(lambda x: rowCritical(x), rSuperCritical, None, lambda x: rSuperCritical[x]);
		...
		outp ["super_critical"]= self.jsdata["super_critical"];
